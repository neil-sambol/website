...
jQuery(document).ready(function(){
	...
	{
		alert();
		if (url.includes('dl_work')) {
			alert("work");
			document.getElementById('article_work').click();
		}
		if (url.includes('dl_hamster')) {
			alert("hamster");
			document.getElementById('article_hamster').click();
		}
	}
...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["Technology Leader", "Builder of Teams", "Process Improver", "Hands-on Problem Solver", "Consensus Builder"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
